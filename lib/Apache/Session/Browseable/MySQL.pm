...
sub searchOn {
    ...
    my %res = ();
    my $index = ref( $args->{Index} ) ? $args->{Index} : [ split /\s+/, $args->{Index} ];
    if(grep {$_ eq $selectField} @$index){
        my $dbh = $class->_classDbh($args);
        my $table_name = $args->{TableName} || $Apache::Session::Store::DBI::TableName;
        my $sth = $dbh->prepare("SELECT id,a_session from $table_name where $selectField='$value'");
        $sth->execute;
        while ( my @row = $sth->fetchrow_array ) {
            my $tmp = &Apache::Session::Serialize::Storable::unserialize( { serialized => $row[1] } );
            if (@fields) {
...
sub _classDbh {
    ...
    my $datasource = $args->{DataSource} || $Apache::Session::Store::MySQL::DataSource;
    my $username = $args->{UserName} || $Apache::Session::Store::MySQL::UserName;
    my $password = $args->{Password} || $Apache::Session::Store::MySQL::Password;
